...
pja_ipmn_geomx_annot_xlsx <- file.path(geomx_dir, "pja_ipmn_wta_annot_2023-04-13.xlsx")
...
#
# patient
p1 <- ggplot(x, aes(x=PC1, y=PC2, color=patient, shape=path_specimen)) +
  geom_point(alpha = 0.8, size=3) +
  scale_color_manual(values = pals::cols25()) +
  theme_bw() +
  theme(legend.position="none") +
  labs(x=res.pca_var_txt[1], y=res.pca_var_txt[2])
p2 <- ggplot(x, aes(x=PC3, y=PC2, color=patient, shape=path_specimen)) +
  geom_point(alpha = 0.8, size=3) +
  scale_color_manual(values = pals::cols25()) +
  theme_bw() +
  labs(x=res.pca_var_txt[3], y=res.pca_var_txt[2])
p <- p1 + p2
f <- file.path(plot_dir, paste0("pca_patient_12_23.pdf"))
...
p2 <- ggplot(x, aes(x=PC3, y=PC2, color=path_aoi, shape=path_specimen)) +
  ...
  labs(x=res.pca_var_txt[3], y=res.pca_var_txt[2])
p <- p1 + p2
f <- file.path(plot_dir, paste0("pca_path_aoi_12_23.pdf"))
ggsave(f, plot=p, device="pdf", width=7, height=3)
...
# scree plot: percent variance of each PC
p <- fviz_eig(res.pca, col.var="blue", addlabels=TRUE)
f <- file.path(plot_dir, paste0("pca_fviz_eig.pdf"))
ggsave(f, plot=p, device="pdf", width=4, height=4)

# choose number of clusters
p <- fviz_nbclust(res.pca$x, FUNcluster=hcut, method="silhouette")
f <- file.path(plot_dir, paste0("pca_fviz_nbclust.pdf"))
ggsave(f, plot=p, device="pdf", width=4, height=4)
...
```{r}
# plot clustering
x <- bind_cols(fds$samples, as_tibble(res.pca$x))
...
p1 <- ggplot(x, aes(x=PC1, y=PC2, color=cluster, shape=path_specimen)) +
  ...
  theme_bw() +
  labs(x=res.pca_var_txt[1], y=res.pca_var_txt[2])
p2 <- ggplot(x, aes(x=PC3, y=PC2, color=cluster, shape=path_specimen)) +
  geom_point(alpha = 0.8, size=3) +
  scale_color_manual(values = color_scales$cluster) +
  theme_bw() +
  labs(x=res.pca_var_txt[3], y=res.pca_var_txt[2])
...
ggsave(f, plot=p, device="pdf", width=width, height=height)
p <- p1 + theme(legend.position="none") + p2
...
p <- ggplot(x, aes(x=PC1, y=PC2, color=cluster, shape=path_specimen, alpha=path_aoi)) +
  geom_point(size=3) +
  scale_alpha_manual(values=alpha_aoi) +
  scale_color_manual(values=color_scales$cluster) +
  theme_bw() +
...
## Identify clusters based on histological patterns

```{r identify clusters}

fds$samples <- fds$samples %>% mutate(
  cluster_name = case_when(cluster == "c1" ~ "cHR",
                           cluster == "c2" ~ "cLR",
                           cluster == "c3" ~ "cNL")
)
...
annot_col <- column_to_rownames(fds$samples, "aoi") %>% 
  select(patient, path_aoi, path_specimen, cluster) %>%
  as.data.frame()
...
for (gene in cluster_marker_genes) {
  print(gene)
  x <- bind_cols(s, as_tibble(res.pca$x), gene=unlist(y[gene,]))
...
contrasts <- c("clusterHRvLR", "clusterHRvNL")

x <- filter(gsea_hallmark, analysis %in% contrasts, pathway %in% pathways)
m <- gsea_leading_edge_matrix2(x, my_padj_cutoff=padj_cutoff, min_gene_sets=1)
...
x <- left_join(x, bind_cols(fds$samples, as_tibble(res.pca$x)), by=join_by(aoi == aoi))
...
x <- bind_cols(fds$samples, as_tibble(res.pca$x), t(gsva_res))
x <- x %>% select(aoi, path_specimen, path_aoi, cluster, all_of(pc_names), all_of(gene_sets))
...
nllgd_samples <- nllgd_samples %>% mutate(
  cluster_nllgd_name = case_when(cluster_nllgd == "c1" ~ "cLR",
                                 cluster_nllgd == "c2" ~ "cHR",
                                 cluster_nllgd == "c3" ~ "cNL")
)
x <- bind_cols(nllgd_samples, as_tibble(nllgd.pca$x))

p <- ggplot(x, aes(x=PC1, y=PC2, color=cluster_nllgd_name, shape=path_specimen)) +
  geom_point(alpha = 0.8, size=3) +
  scale_color_manual(values = color_scales$cluster_name) +
  theme_bw() +
  labs(x=nllgd.pca_var_txt[1], y=nllgd.pca_var_txt[2])
p
f <- file.path(plot_dir, paste0("pca_nllgd_cluster.pdf"))
ggsave(f, plot=p, device="pdf", width=width, height=height)
...
p <- ggplot(gsea, aes(x=NES_cluster1v2, y=NES_cluster1v3, 
  ...
  geom_point(data=filter(gsea, sig_cluster1v3 | sig_cluster1v2), pch=21, size=3, stroke=2) + 
  geom_point(data=filter(gsea, !(sig_cluster1v3 | sig_cluster1v2)), pch=21, size=1, stroke=1) +
