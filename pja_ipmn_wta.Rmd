...
# fds$samples$complexity_norm <- apply(fds$x, 2, function(x) length(unique(x)))
# ggplot(fds$samples, aes(complexity, complexity_norm)) + 
#   geom_point()
...
# contribution
p <- fviz_contrib(pca_res, choice="var", axes = 1:2, top=fviz_ntop) +
  theme(axis.text.x = element_text(angle=90))
f <- file.path(plot_dir, paste0("pca_fviz_contrib12.pdf"))
ggsave(f, p)
...
p <- fviz_gap_stat(cluster::clusGap(x, FUNcluster=cluster::pam, K.max=10, B=100),
...
# CPTAC
res <- fgsea_run_batch(de, gs_cptac, fgsea_eps=0, fgsea_nperms=10000, fgsea_nproc=10)
gsea <- bind_rows(gsea, res)
# HPA
res <- fgsea_run_batch(de, gs_hpa_panc, fgsea_eps=0, fgsea_nperms=10000, fgsea_nproc=10)
gsea <- bind_rows(gsea, res)
# pdacr
res <- fgsea_run_batch(de, gs_pdacr[pdacr_pathways], 
                       fgsea_eps=0, fgsea_nperms=10000, fgsea_nproc=10)
gsea <- bind_rows(gsea, res)
# hallmark
...
gsea <- bind_rows(gsea, res)
# 3ca
res <- fgsea_run_batch(de, gs_3ca, fgsea_eps=0, fgsea_nperms=10000, fgsea_nproc=10)
gsea <- bind_rows(gsea, res)
# gobp
#res <- fgsea_run_batch(de, gs_gobp, fgsea_eps=0, fgsea_nperms=10000, fgsea_nproc=10)
#gsea <- bind_rows(gsea, res)
...
# 3CA
#
res <- gsea %>% filter(pathway %in% names(gs_3ca)) %>% 
  mutate(pathway = gsub("GAVISH_3CA_", "", pathway)) %>%
  mutate(pathway = gsub("METAPROGRAM_", "", pathway))
# only keep significant results (at least one contrast)
res <- res %>% group_by(pathway) %>%
  filter(any(padj < gsea_padj_cutoff)) %>%
  ungroup()
...
ggsave(file.path(plot_dir, paste0(f, ".pdf")), p, width=8, height=12)
ggsave(file.path(plot_dir, paste0(f, ".png")), p, width=8, height=12)
...
#x <- nllgd_samples %>% select(aoi, cluster_nllgd)
#x <- x %>% left_join(select(fds$samples, aoi, cluster_name), by="aoi", suffix=c(".nllgd", ".all"))
...
# CPTAC
res <- fgsea_run_batch(nllgd_de, gs_cptac, fgsea_eps=0, fgsea_nperms=10000, fgsea_nproc=10)
gsea <- bind_rows(gsea, res)
# HPA
res <- fgsea_run_batch(nllgd_de, gs_hpa_panc, fgsea_eps=0, fgsea_nperms=10000, fgsea_nproc=10)
gsea <- bind_rows(gsea, res)
# pdacr
...
gsea <- bind_rows(gsea, res)
# hallmark
res <- fgsea_run_batch(nllgd_de, gs_hallmark, 
                       fgsea_eps=0, fgsea_nperms=10000, fgsea_nproc=10)
gsea <- bind_rows(gsea, res)
# 3ca
res <- fgsea_run_batch(nllgd_de, gs_3ca, fgsea_eps=0, fgsea_nperms=10000, fgsea_nproc=10)
gsea <- bind_rows(gsea, res)
# gobp
#res <- fgsea_run_batch(nllgd_de, gs_gobp, fgsea_eps=0, fgsea_nperms=10000, fgsea_nproc=10)
#gsea <- bind_rows(gsea, res)
...
#
res <- gsea %>% filter(pathway %in% names(gs_3ca)) %>% 
  mutate(contrast = sub("cluster", "", contrast),
         pathway = gsub("GAVISH_3CA_", "", pathway)) %>%
  mutate(pathway = gsub("METAPROGRAM_", "", pathway))
# only keep significant results (at least one contrast)
res <- res %>% group_by(pathway) %>%
  filter(any(padj < gsea_padj_cutoff)) %>%
...
# arrange by clusterHRvLR for readability
pathway_levels <- filter(res, group == "all", contrast == "HRvLR") %>% arrange(NES) %>% pull(pathway)
res$pathway <- factor(res$pathway, levels=pathway_levels)
...
f <- paste0("gsea_nllgd_barplot_clusters_3ca")
ggsave(file.path(plot_dir, paste0(f, ".pdf")), p, width=8, height=12)
ggsave(file.path(plot_dir, paste0(f, ".png")), p, width=8, height=12)
...
g <- "NKX6-2"
s$path_aoi <- factor(s$path_aoi, levels=c("nl", "lgd", "hgd", "inv"))
p <- plot_gene(s, y, g, cluster_name, path_aoi) + 
  scale_fill_manual(values = color_scales$path_aoi)

f <- paste0("gene_plot_", g)
ggsave(file.path(plot_dir, paste0(f, ".pdf")), p, width=3, height=3)
ggsave(file.path(plot_dir, paste0(f, ".png")), p, width=3, height=3)
...
s$path_aoi <- factor(s$path_aoi, levels=c("nl", "lgd", "hgd", "inv"))
p <- plot_gene(s, y, "LAMC2", path_specimen, path_aoi) + 
  scale_fill_manual(values = color_scales$path_aoi)
p
...
x <- gsea %>% filter(pathway %in% pathways)
x <- x %>% mutate(contrast = paste0(contrast, ".", group))

y <- bind_rows(mutate(de, contrast = paste0(contrast, ".all")),
               mutate(nllgd_de, contrast = paste0(contrast, ".nllgd")))

m <- gsea_leading_edge_matrix(x, y, gsea_padj_cutoff=gsea_padj_cutoff, 
                              min_gene_sets=1, min_contrasts=2)
...
h <- 10
w <- 6
save_pheatmap_pdf(p, filename=file.path(plot_dir, "leading_edge_nllgd_basal.pdf"), width=w, height=h)
...
# CPTAC and HPA
pathways <- c(names(gs_cptac), names(gs_hpa_panc))
contrasts <- c("clusterHRvLR", "clusterHRvNL", "clusterLRvNL")

gsea <- bind_rows(gsea_cptac, gsea_hpa_panc)
gsea <- gsea %>%
  filter(pathway %in% pathways, analysis %in% contrasts)
gsea <- mutate(gsea, neglog10padj = ifelse(padj < gsea_padj_cutoff, -log10(padj), NA))
gsea$analysis <- factor(gsea$analysis, levels=contrasts)

p <- ggplot(gsea, aes(x=analysis, y=NES, fill=neglog10padj)) +
  geom_col() + 
  geom_hline(yintercept=0) +
  scale_fill_viridis_c(limits=gsea_colormap_limits, option=gsea_viridis_colormap_option) +
  theme_bw() + 
  theme(axis.text.x = element_text(color = "black", angle = 90, vjust = 0.5, hjust=1)) +
  theme(strip.text = element_text(size=8)) +
  facet_wrap(~ pathway, nrow=1) + 
...
f <- paste0("gsea_barplot_clusters_hallmark")
ggsave(file.path(plot_dir, paste0(f, ".pdf")), p, width=8, height=4)
ggsave(file.path(plot_dir, paste0(f, ".png")), p, width=8, height=4)
...
# arrange for readability
gsea$pathway <- factor(gsea$pathway, levels=pathways)
#gsea$pathway <- factor(gsea$pathway, levels=gsea %>% filter(analysis == "clusterHRvLR") %>% arrange(NES) %>% pull(pathway))
gsea$analysis <- factor(gsea$analysis, levels=c("clusterLRvNL", "clusterHRvNL", "clusterHRvLR"))
...
prefix <- "gsea_cptac"
gs <- gs_cptac
gsea <- run_batch_fgsea(analyses, de, gs, prefix, gsea_plot_dir, gsea_padj_cutoff, do_plots=FALSE)
...
gsea <- run_batch_fgsea(analyses, de, gs_hallmark, prefix, gsea_plot_dir, 
                        gsea_padj_cutoff, do_plots=FALSE)
...
p <- ggplot() +
    ...
    scale_color_manual(values=pals::cols25()) +
    scale_fill_viridis_d(option=viridis_colormap_option) +
    scale_y_log10() +
    theme_bw() +
    theme(axis.text.x = element_blank(),
...
length(intersect(genes, gs_hpa_panc$HPA_PROGNOSIS_PANCREATIC_CANCER_UNFAV_ALL))
# agreement across studies
...
p <- ggplot(x, aes(x=analysis, y=NES, fill=fillneglog10padj)) +
  ...
  scale_fill_viridis_c(option=viridis_colormap_option) +
  labs(x="Analysis", y="Normalized Enrichment Score",
...
p <- ggplot(gsea, aes(x=NES_cluster1v2, y=NES_cluster1v3, 
  ...
  scale_fill_viridis_c(option=viridis_colormap_option, limits=c(2,4), oob=scales::oob_squish) +
  scale_color_viridis_c(option=viridis_colormap_option, limits=c(2,4), oob=scales::oob_squish) +
  geom_vline(xintercept=0) +
  geom_hline(yintercept=0) + 
  xlim(-2.5, 2.5) + 
  ylim(-2.5, 2.5)
