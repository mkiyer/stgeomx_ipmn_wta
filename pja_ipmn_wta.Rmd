...
working_dir <- "/Users/mkiyer/University of Michigan Dropbox/MED-mctp-iyerlab/projects/stgeomx_ipmn_wta/final_submission/analysis"
...
pja_ipmn_geomx_xlsx <- file.path(geomx_dir, "pja_ipmn_wta_geo_2024-09-18.xlsx")
pja_ipmn_geomx_annot_xlsx <- file.path(geomx_dir, "pja_ipmn_wta_geo_annot_2024-09-18.xlsx")
...
shape_scales <- list(
  ...
  path_aoi = c(nl=15, lgd=16, hgd=17, inv=18),
...
s <- fds_duke_panck$samples
y <- fds_duke_panck$log2_norm_cpm
feature_plot_genes <- c("CTRB1", "CRISP3", "TRIM29", "PGC", "CLDN18", "KRT17", "CDH1")
...
#
# immunofluorescence genes
#
w <- 3
h <- 3
s <- fds_panck$samples %>% filter(study == "duke")
y <- fds_panck$log2_norm_cpm[, s$aoi]
genes <- c("CLDN18", "KRT17", "CDH1")

for (g in genes) {
  print(g)
  x <- bind_cols(s, gene=y[g,])
  p <- ggplot(x, aes(x=cluster_name, y=gene)) +
    geom_point(aes(color=patient, shape=path_aoi), 
               position=position_jitter(width=0.1)) +
    geom_boxplot(aes(fill=cluster_name), outlier.size=0, width=0.7, alpha=0.3) + 
    scale_color_manual(values = color_scales$patient) +
    scale_shape_manual(values = shape_scales$path_aoi) +
    scale_fill_manual(values = color_scales$cluster_name) +
    theme_minimal() +
    theme(legend.title=element_text(size=8), 
          legend.text=element_text(size=7)) +
    labs(x="", y="log2 normalized expression", title=g)
  f <- file.path(plot_dir, paste0("duke_if_gene_boxplot_", g))
  ggsave(paste0(f, ".png"), p, width=w, height=h)
  ggsave(paste0(f, ".pdf"), p, width=w, height=h)
}
...
# write gsea results
prefix <- "gsea_nllgd_filtered"
f <- file.path(results_dir, paste0(prefix, "_results.tsv"))
data.table::fwrite(y, file=f, sep="\t", sep2=c("", " ", ""))
f <- file.path(results_dir, paste0(prefix, "_results.xlsx"))
write_xlsx(y, f)
...
```{r write normalized data}

s <- bind_cols(fds_duke_panck$samples, pca_tbl)
s <- left_join(s, select(duke_ipmn_ds$samples, aoi, SegmentDisplayName),
               by=join_by(aoi))

x <- as_tibble(fds_duke_panck$x, rownames="gene")

f <- file.path(results_dir, "duke_panck_norm_counts.xlsx")
write_xlsx(list(aoi_metadata=s, norm_counts=x), f)

s <- bind_cols(fds_duke_stroma$samples, pca_res.stroma$x[,1:2])
x <- as_tibble(fds_duke_stroma$x, rownames="gene")
f <- file.path(results_dir, "duke_stroma_norm_counts.xlsx")
write_xlsx(list(aoi_metadata=s, norm_counts=x), f)
