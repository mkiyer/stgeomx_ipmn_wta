...
get_color_scales <- function(patients) {
  color_scales = list(
    cluster = c(c1="#ff00ff", c2="#ffff00", c3="#4400bb"),
    study = c(ipmn="#ffff00", donor="#ff00ff", soa_panc="#00ff00"),
    path_aoi = c(nl="#0000dd", adm="#008800", panin="#88ff88", lgd="#00ffff",
                 hgd="#ffff00", inv="#ff0000", pdac="#ff0000"),
    path_specimen = c(nl="#0000dd", ipmn_lgd="#0000ff", ipmn_inv="#ff0000",
                      pdac="#ff0000", pnet="#00ff00"),
    path_patient = c(nl_nl="#4B0082", nl_adm="#008800", nl_panin="#00ff00", 
             pdac_nl="#0000cc", pdac_adm ="#00aa00", 
             pdac_panin ="#88ff88", pdac_pdac ="#ff0000",
             ipmn_lgd_nl ="#0000ff", ipmn_lgd_lgd ="#008888",
             ipmn_inv_nl ="#0044ff", ipmn_inv_lgd ="#00ffff",
             ipmn_inv_hgd ="#ffff00", ipmn_inv_inv ="#dd0000",
             pnet_nl = "#0088ff"),
    path = c(nl_nl_epithelial="#4B0082", nl_adm_epithelial="#008800", nl_panin_epithelial="#00ff00", 
             pdac_nl_epithelial="#0000cc", pdac_adm_epithelial="#00aa00", 
             pdac_panin_epithelial="#88ff88", pdac_pdac_epithelial="#ff0000",
             ipmn_lgd_nl_epithelial="#4B0082", ipmn_lgd_lgd_epithelial="#00aa00",
             ipmn_inv_nl_epithelial="#0000ff", ipmn_inv_lgd_epithelial="#00ffff",
             ipmn_inv_hgd_epithelial="#ffff00", ipmn_inv_inv_epithelial="#ff0000",
             pnet_nl_epithelial="#0088ff"),
    de_cptac_rna = c(no="#aaaaaa", dn="#00FFFF", up="#FFFF00", na="#FFFFFF"), 
    de_cptac_prot = c(no="#aaaaaa", dn="#00FFFF", up="#FFFF00", na="#FFFFFF")
  )
...
## IPMN: Read and process IPMN WTA geomx data
...
## IPMN: Filtering thresholds for AOIs/Genes
...
# filter genes
p <- stgeomx::plot_gene_filter(ds, min_frac_expr = geomx_gene_min_frac_expr)
p
...
fds <- stgeomx::bgcorrect(ds, method="qq", bgcorrect_bw_adjust, bgcorrect_bg_quant)
# apply filters
fds <- stgeomx::apply_filters(fds)
# normalize
fds <- stgeomx::normalize(fds, "qn")
...
umap_plot <- function(x, color_by, shape_by) {
  p <- ggplot(x, aes(x=umap1, y=umap2, color={{color_by}}, shape={{shape_by}})) +
...
# run umap
# reproducible UMAP (save seed)
custom.config <- umap.defaults
#custom.config$random_state <- 98723.234
custom.config$random_state <- 56201235.234

umap_fit <- umap(scale(t(y)), config=custom.config)
...
# study
p1 <- umap_plot(umap_df, study, study) +
  scale_color_manual(values = pals::cols25())
f <- file.path(plot_dir, paste0("umap_path_study.pdf"))
ggsave(f, plot=p1, device="pdf", width=width, height=height)

# bg_ks_dist
p1 <- umap_plot(umap_df, bg_ks_dist, path_specimen) +
  scale_color_viridis_c()
f <- file.path(plot_dir, paste0("umap_bg_ks_dist.pdf"))
ggsave(f, plot=p1, device="pdf", width=width, height=height)

# patient
p1 <- umap_plot(umap_df, patient, path_specimen) +
  scale_color_manual(values = color_scales$patient)
f <- file.path(plot_dir, paste0("umap_patient.pdf"))
ggsave(f, plot=p1, device="pdf", width=width, height=height)

# path
p1 <- umap_plot(umap_df, path, path_specimen) +
  scale_color_manual(values = color_scales$path)
f <- file.path(plot_dir, paste0("umap_path.pdf"))
ggsave(f, plot=p1, device="pdf", width=width, height=height)

# specimen
p1 <- umap_plot(umap_df, path_specimen, path_specimen) +
  scale_color_manual(values = color_scales$path_specimen)
f <- file.path(plot_dir, paste0("umap_path_specimen.pdf"))
ggsave(f, plot=p1, device="pdf", width=width, height=height)

# aoi
p1 <- umap_plot(umap_df, path_aoi, path_specimen) +
  scale_color_manual(values = color_scales$path_aoi)
f <- file.path(plot_dir, paste0("umap_path_aoi.pdf"))
ggsave(f, plot=p1, device="pdf", width=width, height=height)
...
fviz_contrib(pca_res$pca, choice = "var", axes = 1, top=50)
# Contributions of variables to PC2
#b<-fviz_contrib(res.pca, choice = "var", axes = 2)


fviz_pca_var(pca_res$pca,
             col.ind = "cos2",
             gradient.cols = c("darkorchid4", "gold", "darkorange"),
             repel=TRUE,
             select.var = list(contrib=50))

fviz_pca_ind(pca_res$pca,
...
# scree plot
fviz_eig(pca_res$pca, addlabels = TRUE)
# choose first 10 PCs
res <- pca_res$pca$x[,1:10]
# show optimal number of clusters
fviz_nbclust(res, FUNcluster=kmeans, k.max = 8) 
fviz_nbclust(res, FUNcluster=kmeans, method="gap_stat", k.max = 8)+ theme_classic()
# cluster into k=3 clusters based on PCs
km <- kmeans(res, centers=3)
...
# memb <- cutree(hc, k=3)
# pca_tbl <- bind_cols(pca_tbl, cluster=paste0("c", memb))

# plot clustering
x <- bind_cols(pca_tbl, cluster=paste0("c", km$cluster))

p1 <- pca_plot(pca_res, x, cluster, path_specimen) + 
  scale_color_manual(values = color_scales$cluster)
f <- file.path(plot_dir, paste0("pca_cluster_specimen.pdf"))
ggsave(f, plot=p1, device="pdf", width=width, height=height)

p1 <- ggplot(x, aes(x=PC1, y=PC2, fill=cluster, shape=path_specimen)) +
  geom_point(color="black", alpha = 0.8, size=3) +
  scale_shape_manual(values = c(21, 24)) +
  scale_fill_manual(values = color_scales$cluster) +
  theme_bw()
f <- file.path(plot_dir, paste0("pca_cluster_specimen.pdf"))
ggsave(f, plot=p1, device="pdf", width=width, height=height)


x <- bind_cols(pca_tbl, cluster=memb)
p1 <- pca_plot(pca_res, x, path, cluster) + 
  scale_color_manual(values = color_scales$path)
f <- file.path(plot_dir, paste0("pca_path_cluster.pdf"))
ggsave(f, plot=p1, device="pdf", width=width, height=height)


# p1 <- ggplot(x, aes(x=PC1, y=PC2, color=cluster, shape=path_specimen)) +
#     geom_point(alpha = 0.8, size=3) +
#     theme_bw() +
#     labs(x=pca_res$pct_var[1], y=pca_res$pct_var[2])
# p2 <- ggplot(x, aes(x=PC2, y=PC3, color=cluster, shape=path_specimen)) +
#     geom_point(alpha = 0.8, size=3) +
#     theme_bw() +
#     labs(x=pca_res$pct_var[2], y=pca_res$pct_var[3])

# d <- bind_cols(t(y), cluster=x$cluster)
# fit <- rpart(cluster ~ ., data=d, method="class")
# randomForest(x=t(y), y=x$cluster)

# plot_gene2(x, log2_norm_cpm, "CRISP3", cluster, path)


gene <- "CRISP3"
x <- bind_cols(pca_tbl, gene=unlist(y[gene,]))
p1 <- pca_plot(pca_res, x, gene, path_specimen) + 
  scale_color_viridis_c()
f <- file.path(plot_dir, paste0("pca_CRISP3_specimen.pdf"))
ggsave(f, plot=p1, device="pdf", width=width, height=height)
...
p1 <- pca_plot(pca_res, x, gene, path_specimen) + 
  scale_color_viridis_c()
f <- file.path(plot_dir, paste0("pca_PGC_specimen.pdf"))
ggsave(f, plot=p1, device="pdf", width=width, height=height)
...
p1 <- pca_plot(pca_res, x, gene, path_specimen) + 
  scale_color_viridis_c()
f <- file.path(plot_dir, paste0("pca_TRIM29_specimen.pdf"))
ggsave(f, plot=p1, device="pdf", width=width, height=height)
...
contrasts <- makeContrasts(cluster1v2 = clusterc1 - clusterc2,
                           cluster1v3 = clusterc1 - clusterc3,
                           cluster2v3 = clusterc2 - clusterc3,
                           levels=design)
...
p <- plot_de_scatter2d(de_merged, 
                       xcontrast="cluster1v3", 
                       ycontrast="cluster1v2", 
                       nlabel=nlabel) +
  labs(title="IPMN Specimens", 
       subtitle="cluster1v3 (x-axis) cluster1v2 (y-axis)")
f <- file.path(plot_dir, "scatter2d_de_x_cluster1v3_y_cluster1v2")
ggsave(paste0(f, ".pdf"), plot=p, width=8, height=8)
ggsave(paste0(f, ".png"), plot=p, width=8, height=8)

p <- plot_de_scatter2d(de_merged, 
                       xcontrast="cluster1v3", 
                       ycontrast="cluster2v3", 
                       nlabel=nlabel) +
  labs(title="IPMN Specimens", 
       subtitle="cluster1v3 (x-axis) cluster2v3 (y-axis)")
f <- file.path(plot_dir, "scatter2d_de_x_cluster1v3_y_cluster2v3")
ggsave(paste0(f, ".pdf"), plot=p, width=8, height=8)
ggsave(paste0(f, ".png"), plot=p, width=8, height=8)
...
```{r ipmn de heatmaps, include=FALSE}
...
annot_col <- column_to_rownames(fds$samples, "aoi") %>% 
  select(study, patient, path_aoi, path_specimen, cluster) %>%
  as.data.frame()
...
```{r ipmn gsva analysis}

# GSVA
y <- log2_norm_cpm
res <- gsva(y, gs_pdacr, method="gsva")
s <- fds$samples
s$path <- factor(s$path, 
                 levels=c("ipmn_lgd_nl_epithelial", 
                          "ipmn_lgd_lgd_epithelial",
                          "ipmn_inv_nl_epithelial",
                          "ipmn_inv_lgd_epithelial",
                          "ipmn_inv_hgd_epithelial",
                          "ipmn_inv_inv_epithelial"))
color_scale <- color_scales$path

x <- bind_cols(s, t(res))
a <- select(x, path, cluster,
            basal=`Moffitt.F6_BasalLike.top100`,
            classical=`Moffitt.F8_Classical.top100`,
            exocrine=`Moffitt.F3_Exocrine.top100`)
a <- a %>% pivot_longer(cols=c("basal", "classical", "exocrine"))

w <- 7
h <- 6

p <- ggplot(a, aes(x=path, y=value, fill=name)) +
  geom_boxplot(outlier.shape=NA, width=0.75) +
  geom_point(position=position_jitterdodge(jitter.width=0.1),
             aes(group=name), pch=21, size=2, alpha=0.8) +
  scale_fill_manual(values = c("basal"="#ff00ff", "classical"="#ffff00", "exocrine"="#4400bb")) +
  labs(title="Moffitt top 100 genes", x="Pathology", y="Gene Set Score (GSVA)") +
  theme_bw() + 
  theme(axis.text.x = element_text(size=6, color="black", angle=90, vjust=0.5, hjust=1),
        axis.line = element_line(color = "black"))
f <- file.path(plot_dir, "gsva_moffitt_path")
ggsave(paste0(f, ".pdf"), p, width=w, height=h)

p <- ggplot(a, aes(x=cluster, y=value, fill=name)) +
  geom_boxplot(outlier.shape=NA, width=0.75) +
  geom_point(position=position_jitterdodge(jitter.width=0.1),
             aes(group=name), pch=21, size=2, alpha=0.8) +
  scale_fill_manual(values = c("basal"="#ff00ff", "classical"="#ffff00", "exocrine"="#4400bb")) +
  ...
  theme(axis.text.x = element_text(size=6, color="black", angle=90, vjust=0.5, hjust=1),
        axis.line = element_line(color = "black"))
f <- file.path(plot_dir, "gsva_moffitt_cluster")
ggsave(paste0(f, ".pdf"), p, width=w, height=h)
...
## IPMN GSEA hallmark pathways across analysis
